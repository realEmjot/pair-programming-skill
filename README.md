...
4. **Per step (agent-written):** propose → you approve/reject/split/ask → agent writes → reports with file:line pointers and, for critical steps, one real question about the behavior → you review the unstaged diff in your editor, edit it if you like → agent reads your edits, adopts them, answers, stages. When there is one obvious way to do a step, the agent skips the proposal and writes it straight away — you still review and approve before it is staged or anything else is written; ask for proposals first whenever you like.
